...
@section('breadcrumb', __('Income & Expense > Expense Categories'))

@section('content')
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    {{-- Create --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 class="text-base font-semibold text-gray-700 mb-4">{{ __('Create Expense Category') }}</h3>
        <form method="POST" action="{{ route('admin.account.expense-category.store') }}">
            @csrf
    ...
    <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <h3 class="text-base font-semibold text-gray-700 p-6 pb-3">{{ __('Expense Category List') }}</h3>
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-3 text-right space-x-2 whitespace-nowrap">
                        ...
                        <form method="POST" action="{{ route('admin.account.expense-category.destroy', $cat) }}" class="inline" onsubmit="return confirm('{{ __('Delete this category?') }}')">
                            @csrf @method('DELETE')
...
<div id="edit-modal" class="fixed inset-0 bg-black/50 z-40 hidden items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-md">
        <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-blue-500 rounded-t-xl">
            <h4 class="text-white font-semibold">{{ __('Edit Expense Category') }}</h4>
...
@push('scripts')
<script>
    const baseUrl = "{{ url('admin/account/expense-category') }}";
