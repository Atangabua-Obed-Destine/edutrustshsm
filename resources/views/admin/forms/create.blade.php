...
@section('breadcrumb', __('Academic > Forms > Create'))

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-700 mb-6">{{ __('New Form') }}</h3>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}"
                               ...
                               placeholder="{{ __('e.g., Form 1') }}" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="short_name" value="{{ old('short_name') }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                               placeholder="{{ __('e.g., F1') }}" required>
                    ...
                    <div>
                        ...
                        <select name="school_level" id="school-level-input" style="appearance: auto; -webkit-appearance: menulist;" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" required>
                            @foreach(\App\Support\LevelContext::availableLabels() as $value => $label)
                                <option value="{{ $value }}" {{ old('school_level', $currentLevel) == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="has_streams" value="1" {{ old('has_streams') ? 'checked' : '' }}
                               class="rounded border-gray-300 text-blue-600 mr-2" id="has-streams-toggle">
                        <span class="text-sm font-medium text-gray-700">{{ __('This form has streams (Science, Arts, etc.)') }}</span>
                    </label>
                </div>

                <div id="streams-section" class="{{ old('has_streams') ? '' : 'hidden' }}">
                    ...
                    <div class="space-y-2">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="streams[]" value="{{ $stream->id }}"
                                   {{ in_array($stream->id, old('streams', [])) ? 'checked' : '' }}
