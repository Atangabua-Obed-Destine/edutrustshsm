...
@section('title', __('Edit Stream'))
@section('breadcrumb', __('Academic > Streams > Edit'))

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-700 mb-6">{{ __('Edit Stream') }} — {{ $stream->name }}</h3>
        ...
        <form method="POST" action="{{ route('admin.streams.update', $stream) }}">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code', $stream->code) }}" maxlength="10"
                ...
                <div>
                    <label class="flex items-center">
                        ...
                        <input type="checkbox" name="is_general" value="1" {{ old('is_general', $stream->is_general) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                              placeholder="{{ __('Brief description of this stream...') }}">{{ old('description', $stream->description) }}</textarea>
                ...
                <div>
                    <label class="flex items-center">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $stream->is_active) ? 'checked' : '' }}
                               class="rounded border-gray-300 text-blue-600">
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t">
                ...
                <button type="submit" class="px-6 py-2 bg-[#1e293b] text-white rounded-lg hover:bg-[#334155] transition text-sm font-medium">{{ __('Update Stream') }}</button>
            </div>
        </form>
    </div>
</div>
