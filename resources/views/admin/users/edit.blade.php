...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Edit Staff Member') }}</h1>
            <p class="mt-1 text-sm text-gray-500">{{ __('Update details for') }} {{ $user->full_name }}</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" placeholder="+237..."
                           class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-2 focus:ring-slate-500 focus:border-slate-500">
                </div>
                <div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    @php
                        ...
                        $isChecked = in_array($r->id, old('roles', $userRoleIds));
                        // Lock the super_admin role if user IS super_admin (cannot strip it)
                        $isLocked = $isSuperAdminRole && $userIsSuperAdmin;
                    @endphp
                    <label class="role-card flex items-start gap-3 p-3 border border-gray-200 rounded-lg {{ $isLocked ? 'opacity-90 cursor-not-allowed bg-slate-50' : 'cursor-pointer hover:border-[#0ea5e9] hover:bg-sky-50/30' }} transition has-[:checked]:border-[#0ea5e9] has-[:checked]:bg-sky-50">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @endif
                                @if($isLocked)
                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-amber-100 text-amber-700 border border-amber-200">{{ __('LOCKED') }}</span>
                                @endif
                            </div>
                            @if($r->description)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('New Password') }} <span class="text-gray-400">{{ __('(leave blank to keep)') }}</span></label>
            ...
            {{-- Account meta info --}}
            <div class="bg-gray-50 rounded-lg p-4 text-sm text-gray-500 grid grid-cols-2 md:grid-cols-4 gap-3">
                <div>
                    <span class="block text-xs text-gray-400 uppercase">{{ __('Status') }}</span>
                    <span class="font-medium {{ $user->is_active ? 'text-green-600' : 'text-red-500' }}">{{ $user->is_active ? __('Active') : __('Inactive') }}</span>
                </div>
                <div>
                    <span class="block text-xs text-gray-400 uppercase">{{ __('Created') }}</span>
                    <span class="font-medium text-gray-700">{{ $user->created_at->format('M d, Y') }}</span>
                </div>
                <div>
                    <span class="block text-xs text-gray-400 uppercase">{{ __('Last Login') }}</span>
                    <span class="font-medium text-gray-700">{{ $user->last_login_at ? $user->last_login_at->diffForHumans() : __('Never') }}</span>
                </div>
                <div>
                    <span class="block text-xs text-gray-400 uppercase">{{ __('Email') }}</span>
